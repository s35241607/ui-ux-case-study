...
13. **多情境展示規範 (Tabs 優先)**：若同一頁面內需要展示多個不同面向或程度的範例，**嚴禁**將多個對比區塊或 Card 長篇上下堆疊。必須統一使用 `Tabs` 元件進行切換。此外，在使用 Tabs 時，包裹對比視圖的 `<SplitView>` 元件必須加上 `:key="activeTab"`（或對應變數），以確保切換情境時，右側「修改後」的探索遮罩能正確恢復。
14. **雙語標題規範**：所有案例的標題 (`h1`、`cases.ts` 中的 `title`) 必須統一附上對應的英文補充，格式一律為「**中文標題 (English Title)**」，例如「視覺間距與舒適度 (Spacing)」。
