  ...
  <div class="h-full flex flex-col">
    <div class="mb-4 px-4 lg:px-6 pt-6">
      ...
      <p class="text-muted-foreground text-sm leading-relaxed max-w-3xl">
        ...
        傳統的「確定/取消」彈窗由於頻繁出現，使用者早已養成<b>「肌肉記憶 (Muscle Memory)」</b>，常常不經思索就盲按確認。
        對於高風險的破壞性操作，必須引入<b>動態認知阻礙</b>（例如：要求手動輸入資源名稱），強迫使用者從自動駕駛模式中醒來。
      </p>
    </div>
    ...
    <SplitView>
      ...
      <template #left>
        <div class="space-y-6 flex flex-col items-center justify-center h-full min-h-[400px]">
          ...
          <!-- 模擬彈窗 -->
          <div v-if="showBadModal" class="fixed rounded-lg border bg-background shadow-lg max-w-sm w-full mx-4 z-50 p-6 animate-in fade-in zoom-in-95" style="position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%);">
          ...
          <!-- Overlay -->
          <div v-if="showBadModal" class="absolute inset-0 bg-background/50 backdrop-blur-sm z-40" @click="showBadModal = false"></div>
      ...
      <template #right>
        <div class="space-y-6 flex flex-col items-center justify-center h-full min-h-[400px] relative">
          ...
          <div class="text-center space-y-4">
            <h3 class="font-medium text-lg">專案設定: {{ projectName }}</h3>
            <Button variant="outline" class="text-destructive hover:bg-destructive hover:text-destructive-foreground" @click="showGoodModal = true">
              <Trash2 class="w-4 h-4 mr-2" />
              刪除專案
            </Button>
          </div>
          ...
          <!-- 安全防呆彈窗 -->
          <div v-if="showGoodModal" class="fixed rounded-lg border border-destructive/20 bg-background shadow-xl max-w-md w-full mx-4 z-50 p-6 animate-in fade-in zoom-in-95" style="position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%);">
            <div class="flex flex-col gap-5">
              <h4 class="font-semibold text-xl">嚴重警告：這項操作極度危險</h4>
              
              <div class="bg-destructive/10 text-destructive text-sm p-4 rounded-md border border-destructive/20 space-y-2">
          ...
          <!-- Overlay -->
          <div v-if="showGoodModal" class="absolute inset-0 bg-background/80 backdrop-blur-sm z-40" @click="cancelGood"></div>
