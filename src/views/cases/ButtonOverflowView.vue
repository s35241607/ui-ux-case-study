              ...
              <div v-for="i in 3" :key="i" class="border-b last:border-0 px-4 py-3">
                <div class="flex flex-col gap-2">
                  ...
                  <div class="flex flex-wrap gap-1">
                    <Button v-for="action in allActions.slice(0, 6)" :key="action.label" size="sm"
                      variant="outline" class="h-6 text-xs px-2"
            ...
            <CardContent>
              <div class="flex items-center gap-2 mb-4">
                ...
                <Button v-for="action in primaryActions" :key="action.label" size="sm"
                  :variant="action === primaryActions[0] ? 'default' : 'outline'"
                  class="h-8"
                ...
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end" class="w-40">
                    <DropdownMenuItem
                      v-for="action in overflowActions.filter(a => !a.destructive)"
                      :key="action.label"
                      @click="() => toast.info(`${action.label}`)"
                      class="gap-2"
                    >
                ...
                <div class="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end" class="w-36">
                      <DropdownMenuItem @click="() => toast.info('分享')" class="gap-2"><Share2 class="h-3.5 w-3.5 text-muted-foreground"/>分享</DropdownMenuItem>
                      <DropdownMenuSeparator />
                      <DropdownMenuItem @click="() => toast.error('刪除')" class="gap-2 text-destructive focus:text-destructive"><Trash2 class="h-3.5 w-3.5"/>刪除</DropdownMenuItem>
                    </DropdownMenuContent>
                  </DropdownMenu>
                </div>
              </div>
